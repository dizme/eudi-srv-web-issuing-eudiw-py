# Pre-authorized

For the pre-authorized flow, you can test our examples by selecting Pre-Authorization Code Grant in https://marmot-civil-gratefully.ngrok-free.app/credential_offer (or <https://localhost/credential_offer> if installed locally).

After submitting, a QR code and DeepLink is generated:

   ![image](./images/preauthorized1.png)

```   
openid-credential-offer://?credential_offer=%7B%22credential_issuer%22:%20%22https://marmot-civil-gratefully.ngrok-free.app/%22%2C%20%22credential_configuration_ids%22:%20%5B%22eu.europa.ec.eudi.loyalty_mdoc%22%5D%2C%20%22grants%22:%20%7B%22urn:ietf:params:oauth:grant-type:pre-authorized_code%22:%20%7B%22pre-authorized_code%22:%20%%22%7D%7D%7D
```
   
